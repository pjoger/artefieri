...
CURLANG = {
	...
	modal_gmap_text: "Введите код карты Google",
	modal_cdata_text: "Введите скриптовый код",
...
//			direction:			"ltr",
			...
			buttons: 			"bold,italic,underline,strike,sup,sub,|,img,link,|,bullist,numlist,smilebox,|,fontcolor,fontsize,fontfamily,|,justifyleft,justifycenter,justifyright,|,quote,code,offtop,table,removeFormat,|,cdata",
			allButtons: {
				bold : {
					title: CURLANG.bold,
				...
				video: {
					...
					transform: {
						'<div><iframe src="http://www.youtube.com/embed/{SRC}" width="640" height="480" frameborder="0"></iframe></div>':'[video="youtube"]{SRC}[/video]'
					}
				},
				gmap: {
					title: CURLANG.gmap,
						...
						tabs: [
							{
								title: CURLANG.gmap,
								input: [
									{param: "MAPSRC",title:CURLANG.modal_gmap_text}
								]
							}
						],
						onSubmit: function(cmd,opt,queryState) {
							var url = this.$modal.find('input[name="MAPSRC"]').val();
							url = url.replace('&amp;', '&');
							var startpos = url.indexOf('src="');
							if (startpos >= 0) {
								var endpos = url.indexOf('">', startpos);
								var code = url.substr(startpos+5, endpos-startpos-5);
								this.insertAtCursor(this.getCodeByCommand(cmd,{mapsrc:code}));
							}
							this.closeModal();
							this.updateUI();
							return false;
					...
					transform: {
						'<div><iframe width="640" height="480" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="{MAPSRC}"></iframe></div>':"[gmap]{MAPSRC}[/gmap]"
					}
				},
				cdata: {
					title: CURLANG.cdata,
					buttonHTML: '<span class="ve-tlb-img"></span>',
					transform: {
						'<div>{SELTEXT}</div>':"[cdata]{SELTEXT}[/cdata]"
					}
				},
